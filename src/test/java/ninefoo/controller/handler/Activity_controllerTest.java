...
public class Activity_controllerTest {
    ...
    @Ignore
    @Test
    public void testLoadActivity_existing_activity() throws Exception {
        // TODO need to implement equals method in activity
        Session.getInstance().setProjectId(1);
        activity_controller.loadActivity(activityID1);
        assertEquals("load existing activity method", "updateLoadActivity", mockUpdatableView.get_called_method());
        assertEquals("load existing activity success", "true", mockUpdatableView.get_success());
        assertEquals("load existing activity message", null, mockUpdatableView.get_message());
        assertEquals("load existing activity object", activity1, mockUpdatableView.get_object_loaded());
    }
